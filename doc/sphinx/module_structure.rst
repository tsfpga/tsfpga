...
The ``.xci`` file is very large and contains much extraneous information, that tends to be updated depending on what computer or Vivado version you're using.
The ``.tcl`` file on the other hand contains only the few settings that are needed.

To find the settings of an existing IP, the following TCL code can be used:
